...
void initPlayground()
{
    ...
    for (int i =0; i<WIDTH;i++)for(int j=0; j<HEIGHT;j++)playground[i][j]=0;
...
void play(Player *p1, Player *p2)
{
    ...
    while (getNextToRead(&c)>0) switch(c){
        ...
        break;
        case 'a':;
        if (p1->prevMove!=RIGHT) p1->nextMove=LEFT;
        break;
        case 's':;
        if (p1->prevMove!=UP) p1->nextMove=DOWN;
        break;
        case 'd':;
        if (p1->prevMove!=LEFT) p1->nextMove=RIGHT;
        break;
        case 'i':;
        if (p2->prevMove!=DOWN) p2->nextMove=UP;
        break;
        case 'j':;
        if (p2->prevMove!=RIGHT) p2->nextMove=LEFT;
        break;
        case 'k':;
        if (p2->prevMove!=UP) p2->nextMove=DOWN;
        break;
        case 'l':;
        if (p2->prevMove!=LEFT) p2->nextMove=RIGHT;
        break;
        
    }

    if (p2->playing)move(p2); //pq en el juego original si se chocan gana p2
